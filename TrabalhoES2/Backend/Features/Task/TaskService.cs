...
public sealed class TaskService : ITaskService
{
    ...
    public async Task<InProgressResultDto> GetInProgressAsync(int userId)
    {
        // filtrar apenas estado “Em curso” (ou string que usares na BD)
           var filter = new TaskFilterDto { 
               Estado     = "Em Curso",
               IsDeleted  = false, 
               Responsavel = userId           // filtrar só as tarefas deste utilizador
           };
           var list = await FilteredListAsync(filter);

        var now = DateTimeOffset.Now;
        var tasksWithDuration = list.Select(t => new
        {
            t.IdTarefa,
            t.Titulo,
            t.Descricao,
            t.Responsavel,
            t.DataInicio,
            Duration = (t.DataFim ?? now) - t.DataInicio
        }).ToList();

        var total = tasksWithDuration.Aggregate(TimeSpan.Zero, (sum, x) => sum + x.Duration);

        // mapear de volta para TaskDetailsDto
        var dtoList = tasksWithDuration.Select(x => _mapper.Map<TaskDetailsDto>(x)).ToList();
    ...
    public async Task<IEnumerable<TaskDetailsDto>> GetCompletedInIntervalAsync(int userId, DateTimeOffset from, DateTimeOffset to)
    {
        var filter = new TaskFilterDto
        {
            Estado = "Concluída",
    ...
    public async System.Threading.Tasks.Task DisassociateTaskFromProjectAsync(int userId, int taskId, int projectId)
    {
        await using var ctx = _factory.CreateDbContext();
